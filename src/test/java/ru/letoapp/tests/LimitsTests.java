...
public class LimitsTests extends SetUpForSuiteBase {
	...
	public void openCardTest() throws Exception {
		...
		appManager.getLimitsScreen().editLimit("ВСЕ РАСХОДНЫЕ ОПЕРАЦИИ",
				"Дневной лимит", "4000");
		appManager.getSpecificLimitScreen().delay();
		appManager.getLimitsScreen().deleteLimit("ВСЕ РАСХОДНЫЕ ОПЕРАЦИИ",
				"Месячный лимит");
		appManager.getSpecificLimitScreen().delay();
		appManager.getLimitsScreen().openLimit("ВСЕ РАСХОДНЫЕ ОПЕРАЦИИ",
				"Годовой лимит");
		appManager.getSpecificLimitScreen().delay();
