...
public class TransfersTests extends SetUpForSuiteBase{
	...
	public void openTransferToBudgetScreenTest() throws Exception {		        
        appManager.getPaymentsAndTransfersScreen().getTransfersTab().transferToTheBudgetBtnClick();
        Assert.assertFalse(appManager.getPaymentsAndTransfersScreen().isErrorPopupDisplayed(), "Payments and transfers screen, open transfer to budget: Error popup displayed");
	}
	...
	public void transferToBudgetTest() throws Exception {		
		...
		appManager.getTransferScreen().chooseFromListOfValues("Основание платежа", "Оплата за школу");
		appManager.getTransferScreen().fillField("Налоговый период (107)", "0");
		appManager.getTransferScreen().fillField("Номер платежного документа (108)", "0");
		appManager.getTransferScreen().fillField("Дата платежного документа (109)", "0");
		appManager.getTransferScreen().chooseFromListOfValues("Тип платежа (110)", "0 - прочее");
		appManager.getTransferScreen().chooseFromListOfValues("Статус плательщика (101)", "16 - участник внешнеэкономической деятельности - физическое лицо");
		appManager.getTransferScreen().fillField("Назначение перевода (24)", "vzyatka");
