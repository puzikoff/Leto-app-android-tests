...
public class TestBase {
	...
	public void chooseEnvironoment(String env) throws Exception {
		if (env.equals("sb")) {
			...
			Assert.assertFalse(appManager.getAuthScreen()
		...
		if (env.equals("mtest")) {
			...
			greetingPopupHandler();
			appManager.getAuthScreen().verifyAuthScreen();
			appManager.getAuthScreen()
					.enterUsername(
							TestsHelper.generateString("qwertyuio123456", 5)
									+ "@mtest");
			appManager.getAuthScreen().enterPassword(
					PropertyReader.getProperty("badPassword"));
			if (appManager.getAuthScreen().isProtCodeCheckboxSelected()) {
	...
	public void incorrectScreenHandler(String title) throws Exception {
		if (!appManager.getDashboardScreen().getTitleFromActionBar()
				.equals(title)) {
